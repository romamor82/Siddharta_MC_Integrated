...
SiddhartaDetectorMessenger::SiddhartaDetectorMessenger(SiddhartaDetectorConstruction* myDet) : myDetector(myDet)
{
...
SiddhartaDetectorMessenger::~SiddhartaDetectorMessenger()
{
  delete TargMatCmd;
  delete ChamMatCmd;
  delete FieldCmd;
...
void SiddhartaDetectorMessenger::SetNewValue(G4UIcommand* command,G4String newValue)
{
  if (command == TargMatCmd) {
    myDetector->setTargetMaterial(newValue);
  } else if (command == FieldCmd) {
