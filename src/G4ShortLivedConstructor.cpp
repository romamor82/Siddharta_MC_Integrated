...
/*
Nothing special here -> To remove
 */
...
//	        "phi",     1019.46*MeV,      4.26*MeV,    +0.0*eplus,
	        "phi",     1019.46*MeV,      1.00*eV,    +0.0*eplus,  // A. Romero
                    2,              -1,            -1,
                    0,              +0,            -1,
              "meson",               0,             0,           333,
		false,             0.0,          NULL);
...
//				                    "kaon+","kaon-");
  mode  = new SinsqDecayChannel("phi",0.492, 2,      // The phi decyas with sin^2(theta) distribution
				                    "kaon+","kaon-");
  decayTable->Insert(mode);
...
//				                   "kaon0S","kaon0L");
  mode  = new SinsqDecayChannel("phi",0.340, 2,
				                   "kaon0S","kaon0L");
  // add decay table
  decayTable->Insert(mode);
...
//  mode  = new G4PhaseSpaceDecayChannel("rho+",1.000, 2,
  mode  = new SinsqDecayChannel("rho+",1.000, 2,
				                    "pi+","pi0");
  // add decay table
  decayTable->Insert(mode);
  particle->SetDecayTable(decayTable);
...
//  mode  = new G4PhaseSpaceDecayChannel("rho0",1.000, 2,
  mode  = new SinsqDecayChannel("rho0",1.000, 2,
				                    "pi+","pi-");
  // add decay table
  decayTable->Insert(mode);
  particle->SetDecayTable(decayTable);
